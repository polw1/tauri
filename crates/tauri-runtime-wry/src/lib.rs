  ...
  I: Iterator<Item = MonitorHandle>,
{
  monitors.find(|monitor| {
    monitor_contains_position(
      monitor.position(),
      monitor.size(),
      window_position.to_physical::<i32>(monitor.scale_factor()),
    )
  })
}
...
mod tests {
  ...
  fn monitor_match_uses_candidate_monitor_scale_factor() {
    ...
    let physical = window_position.to_physical::<i32>(1.5);
    ...
    let physical = window_position.to_physical::<i32>(2.0);
    assert!(!monitor_contains_position(
      monitor_pos,
      monitor_size,
      physical
