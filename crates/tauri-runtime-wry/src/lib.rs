...
impl WindowBuilder for WindowBuilderWrapper {
  ...
  fn with_config(config: &WindowConfig) -> Self {
    ...
    if config.transparent {
      eprintln!(
        "The window is set to be transparent but the `macos-private-api-transparent` feature is not enabled.
        This can be enabled via the `app.macOS.privateApiTransparent` configuration property.
      ");
    }
